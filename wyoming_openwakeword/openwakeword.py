                ...
                if not state.clients_lock.locked() and not state.audio_lock.locked():
                    with state.clients_lock, state.audio_lock:
                        ...
                        if batch_size < 1:
                            ...
                            break
                        _LOGGER.debug("openwakeword 64 %d todos (%d clients)" % (batch_size, len(state.clients)))
                ...
                if not state.clients_lock.locked() and not state.mels_lock.locked():
                    with state.clients_lock, state.mels_lock:
                        ...
                        if batch_size < 1:
                            # Not enough audio to process
                            break
                        _LOGGER.debug("openwakeword 167 %d todos (%d clients)" % (batch_size, len(state.clients)))
                ...
                if not state.clients_lock.locked() and not state.audio_lock.locked():
                    with state.clients_lock, ww_state.embeddings_lock:
                        ...
                        if batch_size < 1:
                            # Not enough audio to process
                            break
                        _LOGGER.debug("openwakeword 284 %d todos (%d clients)" % (batch_size, len(state.clients)))
